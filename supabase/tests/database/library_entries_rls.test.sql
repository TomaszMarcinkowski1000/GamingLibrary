...
--      nothing, and dropping SELECT blocks the write outright. Both measured — see the
--      falsification log below. The effect check earns its place on the two reasons above,
--      not on that scenario.
--
...
--   ------------------------------------------------------------------------------------
--   `for all using (true)` (SELECT + UPDATE widened)  -> 2, 4, 5, 6, 7, 9, 15
--   SELECT policy widened to `using (true)`           -> 2, 8, 9
--   a policy added `to anon`                          -> 16, 17
--   INSERT policy relaxed to `with check (true)`      -> 15
--   RPC user_id predicate dropped + SELECT widened    -> 10 (and 2, 8, 9)
--   `request.jwt.claim.sub` set to a stranger's uuid  -> 1 FIRST, then 2, 3, 9-14
